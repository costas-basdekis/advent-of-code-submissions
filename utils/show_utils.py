...
def show_string_table(table: Dict[Point2D, str], boundaries: Optional[Tuple[Point2D, Point2D]] = None, justify_func: Callable = str.rjust, auto_separator: Union[str, bool] = True, min_cell_length: int = 1) -> str:
    """
    ...
    456
    789
    >>> print(show_string_table(_make([[1, 2, 3], [4, 5, 6], [7, 8, 9]]), min_cell_length=2))
    1  2  3
    4  5  6
    7  8  9
    ...
    (min_x, min_y), (max_x, max_y) = boundaries
    max_length = max(min_cell_length, max(map(len, map(lambda text: click.unstyle(text), table.values()))) if table else 0)
